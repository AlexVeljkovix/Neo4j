...
import { getGamesByAuthorId } from "../api/gameApi";
...
const AuthorDetailsPage = () => {
  ...
  useEffect(() => {
    try {
      ...
      getGamesByAuthorId(id).then((data) => setGames(data));
    } catch (error) {
  ...
  if (!author)
    return (
      <div className="p-8 text-white">
        <p>Nema dostupnih podataka o autoru.</p>
      </div>
    );
  ...
  return (
    <div className="p-8 text-white flex flex-col gap-6">
      ...
      <div className="bg-gray-800 p-6 rounded-lg shadow-md">
        ...
        <p className="text-gray-300">Država: {author.country}</p>
      ...
      {games.length > 0 ? (
        <>
          <h2 className="text-3xl font-bold mb-2 text-black">Igre autora</h2>
        ...
        <div className="flex flex-col items-center justify-center py-20 text-gray-400">
          ...
          <h2 className="text-xl font-semibold mb-2">
            Nema igara za datog autora
          </h2>
          <p className="text-center max-w-sm">
            U bazi ne postoje igre za izabranog autora, izaberite drugog autora
            kako bi videli njegove igre
          </p>
        </div>
      )}
    </div>
